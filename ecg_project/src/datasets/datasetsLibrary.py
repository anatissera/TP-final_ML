...
def get_shaoxing_fmm_dataset(datapath:str="./data", frequency:int=500,lead:int=None, 
    ...
    def get_numpy_dataset_from_slice(slice:str):
        ...
        for row_number,file_name in tqdm.tqdm(enumerate(samples_files),total=len(samples_files)):
            ...
            raw_sample_data = sample["data"]  # This is the data with all leads, e.g., shape (L, 12)

            # Explicitly handle the single-lead case
            if num_leads == 1:
                # We select the first lead (index 0). The brackets ensure the dimension is kept.
                # This changes the shape from (L, 12) -> (L, 1)
                sample_sequence = raw_sample_data[:, [0]] 
            else:
                # If more than 1 lead is requested, use the data as is
                sample_sequence = raw_sample_data
